...
import cn.ennwifi.solu.codes.CodePageException;
import cn.ennwifi.solu.codes.ErrorCodes;
import cn.ennwifi.solu.configure.view.ErrorView;
...
public class GlobalHandlerAdvicer {
  ...
  @ResponseBody
  public ModelAndView requestHandlingNoHandlerFound(HttpServletRequest req,
      NoHandlerFoundException ex) {
    String url = req.getRequestURL().toString();
    return new ErrorView(ErrorCodes.ERROR_RESOURCE_NOT_FIND.code,
        ErrorCodes.ERROR_RESOURCE_NOT_FIND.message);
  }
  ...
  public ModelAndView handlerMethodNotSupportException(HttpServletRequest request,
      Exception exception) {
  ...
  public ModelAndView codePageExceptionErrorHandler(HttpServletRequest request,
      CodePageException exception) {
    ModelMap mm = new ModelMap();
    ModelAndView mav;
    if (exception.getCode() == ErrorCodes.ERROR_LOGIN.code) {
  ...
  public ModelAndView handleBindError(HttpServletRequest request,
  ...
  public ModelAndView defaultErrorHandler(HttpServletRequest request, Exception exception) {
    ...
    if (stackTrace.length > 0) {
      ...
      msg +=
          "\r\n错误代码在" + ste.getClassName() + "类:" + ste.getMethodName() + "方法的"
              + ste.getLineNumber() + "行";
    }
